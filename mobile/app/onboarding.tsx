 ...
 * Steps: Language → Name → Location → Done (simplified)
 */
...
const P: Record<string, { name: string; loc: string; welcome: string; tapMic: string }> = {
  kn: { name: 'ನಿಮ್ಮ ಹೆಸರು ಏನು?', loc: 'ದಯವಿಟ್ಟು ನಿಮ್ಮ ಜಿಲ್ಲೆ ಹೇಳಿ', welcome: 'ಸ್ವಾಗತ', tapMic: 'ಮೈಕ್ ಒತ್ತಿ ಮಾತನಾಡಿ' },
  hi: { name: 'आपका नाम क्या है?', loc: 'कृपया अपना जिला बताइए', welcome: 'स्वागत है', tapMic: 'माइक दबाकर बोलें' },
  ta: { name: 'உங்கள் பெயர் என்ன?', loc: 'தயவுசெய்து உங்கள் மாவட்டத்தை சொல்லுங்கள்', welcome: 'வரவேற்கிறோம்', tapMic: 'மைக்கை அழுத்தி பேசுங்கள்' },
  te: { name: 'మీ పేరు ఏమిటి?', loc: 'దయచేసి మీ జిల్లాను చెప్పండి', welcome: 'స్వాగతం', tapMic: 'మైక్ నొక్కి మాట్లాడండి' },
  ml: { name: 'നിങ്ങളുടെ പേര്?', loc: 'ദയവായി നിങ്ങളുടെ ജില്ല പറയൂ', welcome: 'സ്വാഗതം', tapMic: 'മൈക്ക് അമർത്തി സംസാരിക്കൂ' },
  mr: { name: 'तुमचे नाव काय?', loc: 'कृपया तुमचा जिल्हा सांगा', welcome: 'स्वागत', tapMic: 'मायक दाबून बोला' },
  bn: { name: 'আপনার নাম কি?', loc: 'দয়া করে আপনার জেলা বলুন', welcome: 'স্বাগতম', tapMic: 'মাইক চাপুন এবং বলুন' },
  gu: { name: 'તમારું નામ શું છે?', loc: 'કૃપા કરીને તમારો જિલ્લો કહો', welcome: 'સ્વાગત', tapMic: 'માઇક દબાવી બોલો' },
  pa: { name: 'ਤੁਹਾਡਾ ਨਾਂ ਕੀ ਹੈ?', loc: 'ਕਿਰਪਾ ਕਰਕੇ ਆਪਣਾ ਜ਼ਿਲ੍ਹਾ ਦੱਸੋ', welcome: 'ਜੀ ਆਇਆਂ', tapMic: 'ਮਾਈਕ ਦਬਾਓ ਅਤੇ ਬੋਲੋ' },
  od: { name: 'ଆପଣଙ୍କ ନାମ?', loc: 'ଦୟାକରି ଆପଣଙ୍କ ଜିଲ୍ଲା କୁହନ୍ତୁ', welcome: 'ସ୍ୱାଗତ', tapMic: 'ମାଇକ୍ ଦବାଇ କୁହନ୍ତୁ' },
  en: { name: 'What is your name?', loc: 'Please say your district', welcome: 'Welcome', tapMic: 'Tap mic and speak' },
};
...
export default function OnboardingScreen() {
  ...
  async function handleRecord() {
    if (recording) {
      try {
        ...
        if (!transcript) { setStatus(prompt.tapMic); return; }
      ...
      try { await startRecording(); setRecording(true); setStatus('🎙️ ...'); }
        ...
        Alert.alert(langCode === 'en' ? 'Permission Denied' : 'ಅನುಮತಿ ನಿರಾಕರಿಸಲಾಗಿದೆ',
  ...
  async function processTranscript(transcript: string) {
    try { await speakText(transcript, sarvam); } catch {}
