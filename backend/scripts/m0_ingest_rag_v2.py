...
DOCUMENTS = [
    ...
    # JSONs
    {
        ...
        "source_doc": "Organic Inputs Knowledge Base",
        "category": "biofertiliser",
        "crop_tag": "general",
        "zone_tag": "all",
        "language": "en",
    ...
    },
    {
        ...
        "source_doc": "Mulching Plants Knowledge Base",
        "category": "mulching",
        "crop_tag": "general",
        "zone_tag": "all",
        "language": "en",
    ...
    },
    {
        ...
        "source_doc": "Karnataka Soil Zones",
        "category": "soil_fertility",
        "crop_tag": "general",
        "zone_tag": "all",
        "language": "en",
        ...
        with pdfplumber.open(path) as pdf:
            for i, page in enumerate(pdf.pages):
                ...
                if text and len(text.strip()) >= 50:
                    ...
                    print(f"  [SKIP] Skipping page {i+1} (too short or empty)")
...
def ingest_document(doc, model, supabase):
    ...
    if not os.path.exists(path):
        print(f"  [SKIP] File not found, skipping: {path}")
        return 0
    ...
    if not units:
        print(f"  [SKIP] No text extracted, skipping")
        return 0

    print(f"  [CHUNKS] {len(units)} chunks created")
    ...
    for i in range(0, len(units), EMBED_BATCH_SIZE):
        ...
        for unit, embedding in zip(batch, embeddings):
            ...
            # Construct metadata with all document context
            metadata = {
            ...
            # Build row with only basic columns that should exist
    ...
    for i in range(0, len(rows), UPLOAD_BATCH_SIZE):
        ...
        try:
            ...
            print(f"  [WARN] Upsert failed, trying insert...")
            try:
                ...
                print(f"  [DEBUG] Batch sample keys: {list(batch[0].keys()) if batch else 'empty'}")
                # Try with just content and embedding as fallback
                minimal_batch = []
                for item in batch:
                ...
                try:
                    supabase.table("document_chunks").insert(minimal_batch).execute()
                    uploaded += len(minimal_batch)
                    print(f"  [UP] Uploaded {uploaded}/{len(rows)} chunks (minimal schema)")
                except Exception as fallback_e:
                    print(f"  [ERROR] Minimal insert also failed: {fallback_e}")
...
def main():
    ...
    # Load model
    print("\n[LOADING] Loading sentence-transformers model...")
    ...
    print("\n" + "=" * 50)
    print(f"[COMPLETE] Ingestion complete! Total chunks uploaded: {total_chunks}")
